...
BEGIN
  ...
  FOR i IN 1..8 LOOP
    ...
    v_cityx := cities(1+MOD(i,cities.COUNT));
    v_field := fieldcat(fieldcodes(1+MOD(i,fieldcodes.COUNT)));
    INSERT INTO RE_TRAINING_PROGRAMS(org_id,title,city,seats_total,original_fee,discount_pct,is_free,status,published_at,field_cat,start_date,duration_hours,prog_level)
